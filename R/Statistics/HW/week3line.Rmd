...
<li> Yes, revise wording to "Does beauty score decrease the professor's course evaluations?" </li>a
</ol>
...
<li> The left skewness of the data suggests that the students are less likely to rate the professors highly. </li>d
</ol>
```{r score-dist}
...
res=resid(a)
ggplot(data=evals,aes(x=bty_avg,y=res))+geom_smooth(method='lm')
hist(res)
```
...
b <- lm(score ~ bty_avg + gender, data = evals)
summary(m_bty_gen)
```
...
rr=resid(b)
hist(rr)
ggplot(data=evals,aes(x=bty_avg, y=rr))+geom_smooth(method='lm')
```
...
```{r new-mlr-model}
# type your code for the Exercise here, and Knit
cc=lm(score~bty_avg+rank,data=evals)
summary(cc)
...
summary(m2)
m2 = lm(score ~ rank + gender + language + age + cls_perc_eval  + cls_level + cls_profs + cls_credits + bty_avg, data = evals)
summary(m2)
